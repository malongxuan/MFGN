#-*-coding:utf-8-*-
import theano #add for scan
import theano.tensor as T #add for T.
...
class ModelFactory(object):
	...
	def get_listwise_model(model_param, embedding_file, vocab_size):
		def get_core_model(model_param, embedding_file, vocab_size):
			...
			ques_intra = Attend_q(question_emb,question_emb)#25*25
			ans_intra = Attend_a(answer_emb,answer_emb)#90*90
			...
			print answer_Max._keras_shape#(1,300)
			print answer_Mean._keras_shape#(1,300)
			...
			print("question_intra_Max._keras_shape",question_intra_Max._keras_shape)#(1,300)
			print("answer_intra_Max._keras_shape",answer_intra_Max._keras_shape)#(1,300)
			...
			print answer_intra_Soft._keras_shape#(92,300)
			...
			print answer_emb._keras_shape#(98,300)
			...
			print answer_proj._keras_shape#(92,300)

			#=====second co att
			negpool = Lambda(lambda x: -x)
			Attend2 = _Attention(model_param.enc_timesteps+8, model_param.dec_timesteps+8, hidden_dim)
			...
			answer_align = Align(question_proj,ques_atten_metrics,model_param.dec_timesteps+8)
			question_align = Align(answer_proj,ans_atten_metrics,model_param.enc_timesteps+8)
			print answer_align._keras_shape
			#========compare
			ans_sim_output_co = merge([answer_proj,negpool(answer_align)],mode="sum")
			ques_sim_output_co = merge([question_proj,negpool(question_align)],mode="sum")

			##======second intra
			Attend_q2 = _Attention(model_param.enc_timesteps+8, model_param.enc_timesteps+8, hidden_dim)
			Attend_a2 = _Attention(model_param.dec_timesteps+8, model_param.dec_timesteps+8, hidden_dim)
			Align2 = _SoftAlignment(hidden_dim)
			#co-att and intra-att
			ques_atten_metrics_intra = Attend_q2(question_proj,question_proj)#25*25
			ans_atten_metrics_intra = Attend_a2(answer_proj,answer_proj)#90*90
			question_intra_align = Align(question_proj,ques_atten_metrics_intra,model_param.enc_timesteps+8)
			answer_intra_align = Align(answer_proj,ans_atten_metrics_intra,model_param.dec_timesteps+8)
			
			ques_sim_output_intra = merge([question_proj,negpool(question_intra_align)],mode="sum")
			ans_sim_output_intra = merge([answer_proj,negpool(answer_intra_align)],mode="sum")

			#concate second co and intra-att
			ans_sim_output = merge([ans_sim_output_co,ans_sim_output_intra],mode="concat",concat_axis=-1)
			ques_sim_output = merge([ques_sim_output_co,ques_sim_output_intra],mode="concat",concat_axis=-1)
			###################
			...
			FinalDense = Dense(hidden_dim/10,activation="tanh")
